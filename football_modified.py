...
def popularity(selectname):
    ...
    rate={}
    year_arr=[]
    f = open("/Users/Keevey/Documents/Explore/Baby_name/us-baby-names/StateNames.csv")
    ...
    for i in range(len(year_arr)):
    	rate[year_arr[i]] = (int(sumname[year_arr[i]]) - int(sumname[year_arr[i-1]]))/ int(sumname[year_arr[i-1]])
    for key,values in sorted(rate.items(), key=operator.itemgetter(1),reverse=True): #sort by value:
        return key
...
for i in range(0,len(Names)):
    ...
    if popyear is None:
        continue
    print("The Name " + str(Names[i]) +" is significantly increasing during the year " + str(popyear))
    influenceStart = int(startYear[i])-3
    influenceEnd = int(endYear[i])+2
...
print("Names with highest rate of change when the current MVP with same name is :") 
print(selectedNames)
